...
- (void)buildPostBody
{
	...
	} else if ([self postFormat] == ASITextEncodedPostFormat) {
        [self buildTextEncodedPostBody];
    } else {
...
#if DEBUG_FORM_DATA_REQUEST
	[self addToDebugBody:@"\r\n==== Building an text/plain body ====\r\n"]; 
#endif
	
	
	NSString *charset = (NSString *)CFStringConvertEncodingToIANACharSetName(CFStringConvertNSStringEncodingToEncoding([self stringEncoding]));
    
//	[self addRequestHeader:@"Content-Type" value:[NSString stringWithFormat:@"text/plain; charset=%@",charset]];
    [self addRequestHeader:@"Content-Type" value:[NSString stringWithFormat:@"application/x-www-form-urlencoded; charset=%@",charset]];
